...
Dept.openAddDept = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: HuiShu.ctxPath + '/dept/dept_add'
    });
...
Dept.openDeptDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: HuiShu.ctxPath + '/dept/dept_update/' + Dept.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Dept.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(HuiShu.ctxPath + "/dept/delete", function () {
                HuiShu.success("删除成功!");
                Dept.table.refresh();
            }, function (data) {
        ...
        HuiShu.confirm("是否刪除该部门?", operation);
    }
};
