...
DeptInfoDlg.addSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(HuiShu.ctxPath + "/dept/add", function(data){
        HuiShu.success("添加成功!");
...
DeptInfoDlg.editSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(HuiShu.ctxPath + "/dept/update", function(data){
        HuiShu.success("修改成功!");
        window.parent.Dept.table.refresh();
        DeptInfoDlg.close();
    },function(data){
...
$(function() {
    HuiShu.initValidator("deptInfoForm", DeptInfoDlg.validateFields);
