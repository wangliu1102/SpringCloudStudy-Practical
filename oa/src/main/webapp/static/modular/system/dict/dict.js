...
Dict.openAddDict = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: HuiShu.ctxPath + '/dict/dict_add'
    });
...
Dict.openDictDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: HuiShu.ctxPath + '/dict/dict_edit/' + Dict.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Dict.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(HuiShu.ctxPath + "/dict/delete", function (data) {
                HuiShu.success("删除成功!");
                Dict.table.refresh();
            }, function (data) {
        ...
        HuiShu.confirm("是否刪除字典 " + Dict.seItem.name + "?", operation);
    }
};
