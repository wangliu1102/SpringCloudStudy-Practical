...
Role.openAddRole = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: HuiShu.ctxPath + '/role/role_add'
    });
...
Role.openChangeRole = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: HuiShu.ctxPath + '/role/role_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Role.delRole = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(HuiShu.ctxPath + "/role/remove", function () {
                HuiShu.success("删除成功!");
                Role.table.refresh();
            }, function (data) {
        ...
        HuiShu.confirm("是否删除角色 " + Role.seItem.name + "?",operation);
    }
};
...
Role.assign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: HuiShu.ctxPath + '/role/role_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Role.dateAuthority = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: HuiShu.ctxPath + '/role/role_DateAuthority/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
