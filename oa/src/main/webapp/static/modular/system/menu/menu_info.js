...
MenuInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(HuiShu.ctxPath + "/menu/add", function (data) {
        HuiShu.success("添加成功!");
...
MenuInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(HuiShu.ctxPath + "/menu/edit", function (data) {
        HuiShu.success("修改成功!");
        window.parent.Menu.table.refresh();
        MenuInfoDlg.close();
    }, function (data) {
...
MenuInfoDlg.showMenuSelectTree = function () {
    HuiShu.showInputTree("pcodeName", "pcodeTreeDiv", 15, 34);
};

$(function () {
    HuiShu.initValidator("menuInfoForm", MenuInfoDlg.validateFields);
