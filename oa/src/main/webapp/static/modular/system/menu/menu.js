...
Menu.openAddMenu = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: HuiShu.ctxPath + '/menu/menu_add'
    });
...
Menu.openChangeMenu = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: HuiShu.ctxPath + '/menu/menu_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Menu.delMenu = function () {
    if (this.check()) {
        ...
        var operation = function () {
            var ajax = new $ax(HuiShu.ctxPath + "/menu/remove", function (data) {
                HuiShu.success("删除成功!");
                Menu.table.refresh();
            }, function (data) {
        ...
        HuiShu.confirm("是否刪除该菜单?", operation);
    }
};
/**
 * 展开/折叠
