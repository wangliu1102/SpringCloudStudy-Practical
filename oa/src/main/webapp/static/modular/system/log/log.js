...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(HuiShu.ctxPath + "/log/detail/" + this.seItem.id, function (data) {
            HuiShu.infoDetail("日志详情", data.regularMessage);
        }, function (data) {
            HuiShu.error("获取详情失败!");
        });
        ajax.start();
...
OptLog.delLog = function () {
    HuiShu.confirm("是否清空所有日志?",function(){
        var ajax = HuiShu.baseAjax("/log/delLog","清空日志");
        ajax.start();
