...
Notice.openAddNotice = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: HuiShu.ctxPath + '/notice/notice_add'
    });
...
Notice.openNoticeDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: HuiShu.ctxPath + '/notice/notice_update/' + Notice.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Notice.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(HuiShu.ctxPath + "/notice/delete", function (data) {
                HuiShu.success("删除成功!");
        ...
        HuiShu.confirm("是否删除通知 " + Notice.seItem.title + "?", operation);
    }
};
