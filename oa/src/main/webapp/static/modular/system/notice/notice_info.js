...
NoticeInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(HuiShu.ctxPath + "/notice/add", function (data) {
        HuiShu.success("添加成功!");
...
NoticeInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(HuiShu.ctxPath + "/notice/update", function (data) {
        HuiShu.success("修改成功!");
        window.parent.Notice.table.refresh();
        NoticeInfoDlg.close();
    }, function (data) {
...
$(function () {
    HuiShu.initValidator("noticeInfoForm", NoticeInfoDlg.validateFields);
