...
MgrUser.openAddMgr = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: HuiShu.ctxPath + '/mgr/user_add'
    });
...
MgrUser.openChangeUser = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: HuiShu.ctxPath + '/mgr/user_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.roleAssign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: HuiShu.ctxPath + '/mgr/role_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.delMgrUser = function () {
    if (this.check()) {
        var operation = function () {
            ...
            var ajax = new $ax(HuiShu.ctxPath + "/mgr/delete", function () {
                HuiShu.success("删除成功!");
        ...
        HuiShu.confirm("是否删除用户" + MgrUser.seItem.account + "?", operation);
    }
};
...
MgrUser.freezeAccount = function () {
    if (this.check()) {
        ...
        var ajax = new $ax(HuiShu.ctxPath + "/mgr/freeze", function (data) {
            HuiShu.success("冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
            HuiShu.error("冻结失败!" + data.responseJSON.message + "!");
        });
        ajax.set("userId", userId);
        ajax.start();
    }
...
MgrUser.unfreeze = function () {
    if (this.check()) {
        var userId = this.seItem.id;
        var ajax = new $ax(HuiShu.ctxPath + "/mgr/unfreeze", function (data) {
            HuiShu.success("解除冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
            HuiShu.error("解除冻结失败!");
        });
...
MgrUser.resetPwd = function () {
    if (this.check()) {
        var userId = this.seItem.id;
        parent.layer.confirm('是否重置[' + this.seItem.name + ']密码？', {
            btn: ['确定', '取消'],
            shade: false //不显示遮罩
        }, function () {
            var ajax = new $ax(HuiShu.ctxPath + "/mgr/reset", function (data) {
                HuiShu.success("重置密码成功，密码为" + data.data);
            }, function (data) {
                HuiShu.error("重置密码失败!");
            });
            ajax.set("userId", userId);
...
MgrUser.exportExcel = function () {
    $.ajax({
        url: HuiShu.ctxPath + '/mgr/exportExcel',
        type: "POST",
        dataType: "json",
